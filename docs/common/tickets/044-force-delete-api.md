...
- [ ] 指定したユーザーを即座に削除するAPIを実装
- [ ] 削除理由の入力を必須とする
- [ ] superAdminのみが実行可能とする（adminは不可）
- [ ] 削除前に確認ダイアログを表示する仕組みを用意
- [ ] 削除操作を監査ログに記録

### 関連データの完全削除

- [ ] Firebase Authentication からユーザーを削除
- [ ] Firestore の users ドキュメントを削除
- [ ] サブコレクション（sessions, consents）を削除
- [ ] BigQuery の関連データを匿名化
- [ ] Stripe の顧客データを削除（該当する場合）
- [ ] Cloud Storage の関連ファイルを削除（該当する場合）

### 削除処理の安全性確保

- [ ] トランザクション処理を実装（部分削除を防ぐ）
- [ ] 削除失敗時のロールバック機能を実装
- [ ] 削除前のデータバックアップを作成
