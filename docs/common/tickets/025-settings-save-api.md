...
- [x] 設定保存APIを実装（`settings_updateSettings`）
- [x] 設定取得APIを実装（`settings_getSettings`）
- [x] 音声設定の保存・取得機能（音量、速度、ON/OFF）
- [x] 表示設定の保存・取得機能（テーマ、カメラガイド表示）
- [x] 設定値のバリデーション（音量0-100、速度0.5-2.0など）
- [x] 設定の初期値を定義
- [x] レート制限の実装（50回/時）
...
- `functions/src/api/settings/update.ts` - 設定更新API
- `functions/src/api/settings/get.ts` - 設定取得API
- `functions/src/api/settings/index.ts` - エクスポート
