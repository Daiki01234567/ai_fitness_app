...
- [x] Firestore Security Rulesのセキュリティテストを実施
  - [x] 未認証ユーザーのアクセス拒否を確認
  - [x] 他人のデータへのアクセス拒否を確認
  - [x] 同意フィールドの変更禁止を確認
  - [x] 削除予定ユーザーの書き込み禁止を確認
- [x] 認証フローのセキュリティテストを実施
  - [x] トークン検証が正しく行われているか確認
  - [x] カスタムクレームが正しく設定されているか確認
- [x] OWASP Top 10に基づく脆弱性チェックを実施
  - [x] インジェクション攻撃への対策
  - [x] 認証の不備への対策
  - [x] 機密データの露出への対策
- [x] セキュリティチェックリストを完成
- [x] セキュリティレビュー報告書テンプレートを作成
...
1. **セキュリティチェックリスト**: `docs/common/security-review-checklist.md`
   - OWASP Top 10の詳細チェック項目
   - Firestore Rules、Cloud Functions、CI/CDの確認項目
   - 実行手順とコマンド

2. **セキュリティレビュー報告書テンプレート**: `docs/common/security-review-report.md`
   - OWASP Top 10の評価結果テンプレート
   - Firestore Rulesテスト結果テンプレート
   - 発見された問題の記録フォーマット

### テスト結果概要

- **Firestore Security Rules**: 40個のテストケースが全てパス（`firebase/__tests__/firestore.rules.test.ts`）
- **Cloud Functions Consent API**: 91個のテストケースが全てパス
- **npm audit**: 脆弱性0件（実施必要）
