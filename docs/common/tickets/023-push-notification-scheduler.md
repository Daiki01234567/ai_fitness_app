...
- [x] トレーニングリマインダーの時刻設定を保存する機能を実装
- [x] ユーザーごとのリマインダー時刻を管理する機能を実装
- [x] Cloud Schedulerでトリガーされるリマインダー送信関数を実装
- [x] 削除予定ユーザーに30日前、7日前、1日前の通知を送信する機能を実装
- [x] 削除予定日を毎日チェックするスケジュール関数を実装（午前4時UTC）
- [x] リマインダー設定のON/OFF機能を実装
- [x] タイムゾーンを考慮した通知時刻の計算を実装
...
- `functions/src/api/notification/scheduler.ts` - スケジュール関数
  - `notification_sendTrainingReminders`: 毎時実行、トレーニングリマインダー送信
  - `notification_sendDeletionNotices`: 毎日4時UTC実行、削除通知送信
- `functions/src/services/notificationService.ts` - 通知送信サービス
