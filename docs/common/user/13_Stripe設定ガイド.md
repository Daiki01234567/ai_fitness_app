...
6. [領収書設定（日本語・会社情報）](#6-領収書設定日本語会社情報)
7. [テスト環境と本番環境の切り替え](#7-テスト環境と本番環境の切り替え)
...
Stripeで発行される領収書（請求書）の日本語化と、会社情報の設定を行います。

### なぜこの設定が必要？

- 日本のユーザーに読みやすい領収書を提供するため
- 税務要件に対応するため（会社名・住所の表示）
- アプリから自動送信される領収書メールの品質向上
...
2. **会社名（Business name）を設定**

   - 日本語で入力可能です（例：「株式会社AIフィットネス」）
   - 領収書の上部に表示されます
...
2. **ビジネス情報を入力**
   ...
   | 項目 | 説明 | 例 |
   |------|------|-----|
   | Business name | 会社名・事業者名 | 株式会社AIフィットネス |
   | Support email | サポートメール | support@ai-fitness.example.com |
   | Support phone | 電話番号 | +81-3-xxxx-xxxx |
   | Business address | 事業所住所 | 〒xxx-xxxx 東京都... |
...
**重要**：このアプリでは独自の領収書メール送信機能を実装しています。
Stripeの自動メール送信と重複しないように設定してください。

1. **Customer emails ページを開く**
...
2. **設定の選択**
   ...
   **方法A: Stripeの自動送信を無効化（推奨）**
   - 「Successful payments」のチェックを外す
   - アプリ側の独自メールのみ送信されます

   **方法B: 両方とも有効にする**
   - Stripeとアプリから2通のメールが届きます
   - 確実に届くので安全ですが、ユーザーにとっては少し煩わしいかもしれません

### 6.4 領収書言語設定

1. **Customer portal settings を開く**
...
1. テストモードで決済を行う
2. Stripeダッシュボードの「支払い」から該当の支払いを選択
3. 「請求書を表示」をクリック
4. PDFがダウンロードされ、設定した内容が反映されているか確認
