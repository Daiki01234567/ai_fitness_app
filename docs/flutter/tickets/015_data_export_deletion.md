...
## MVP実装方針（2025-12-05更新）

### データエクスポート
- **MVP**: PDF形式で即時ダウンロード（Flutter側で生成）
- **管理者機能**: JSON/CSV、メール通知、非同期処理

### データ削除
- **MVP**: 削除完了時に「削除完了しました」画面表示
- **管理者機能**: 削除証明書、メール通知

### 保留項目（管理者機能実装時に対応）
- SendGrid導入（メール通知）
- 72時間以内完了の運用検証（※GDPR要件は1ヶ月以内）
- 削除証明書発行（※GDPR法的要件ではなくベストプラクティス）
...
- [x] エクスポートリクエスト画面
  ...
  - [x] フォーマット選択
    - [x] JSON（管理者機能で対応）
    - [x] CSV（管理者機能で対応）
    - [x] PDF（MVP - Flutter側で即時生成）実装完了
  - [x] 配信方法
    - [ ] メール送信（管理者機能で対応）
    - [x] アプリ内ダウンロード（MVP - 即時ダウンロード）
    - [ ] クラウドストレージ連携（対象外）
- [x] リクエスト確認
  - [ ] 本人確認（再認証）
  - [x] 処理時間の説明
...
### MVP受け入れ条件
- [ ] PDFエクスポートが即時完了（Flutter側で生成）
- [ ] 全データがPDFに含まれることを確認
- [ ] 削除完了時に「削除完了しました」画面が表示される
- [ ] 削除後にデータが残存しない
- [ ] 30日以内なら復元可能

### 管理者機能受け入れ条件（保留）
- [ ] JSON/CSVエクスポートが1ヶ月以内に完了（GDPR第12条3項）
- [ ] メール通知が送信される
- [ ] 削除証明書が発行される
- [ ] 監査ログが適切に記録される
...
Flutter側でPDF生成を行うため、実装作業が必要です。

| 項目 | 説明 | 所要時間 |
|------|------|---------|
| **`pdf`パッケージ追加** | `flutter pub add pdf` | 1分 |
| **`path_provider`追加** | `flutter pub add path_provider` | 1分 |
| **PDF生成サービス実装** | データ収集→PDF生成→保存 | 2-3時間 |
| **UI統合** | ダウンロードボタン追加 | 30分 |

**実装指示**: 「PDFエクスポート機能を実装してください」とお伝えください。

#### 2. 削除完了画面の実装確認

| 項目 | 説明 |
|------|------|
| **確認内容** | 削除完了時に「削除完了しました」画面が表示されるか |
| **期待動作** | 削除処理完了後、確認画面を表示してログアウト |

---

### 管理者機能実装時に対応（保留）

以下は管理者機能実装フェーズで対応予定です。

#### SendGrid設定（保留）

| 項目 | 説明 | 所要時間 |
|------|------|---------|
| **SendGridアカウント作成** | [SendGrid](https://sendgrid.com/)でアカウント作成 | 10分 |
| **APIキー取得** | Settings > API Keys > Create API Key | 5分 |
| **送信者認証** | Settings > Sender Authentication > ドメイン認証 | 15分 |
| **環境変数設定** | Firebase FunctionsにAPIキーを設定 | 5分 |

#### 削除証明書（保留）

- 署名付き削除証明書は**GDPR法的要件ではなくベストプラクティス**
- 管理者機能フェーズで実装予定

#### 72時間以内完了検証（保留）

- **注意**: GDPR要件は**1ヶ月以内**（第12条3項）
- 72時間は厳しすぎる目標設定のため、管理者機能フェーズで再検討
