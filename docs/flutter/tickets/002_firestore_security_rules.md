...
1. **poseDataサイズ制限テスト**
   - 理由: 設計上、poseDataは `sessions/{sessionId}/frames` サブコレクションに保存される構造のため、親ドキュメントでのサイズ制限テストは不要

2. **削除予定ユーザーのセッション作成制限テスト**
   - 理由: エミュレータ環境での `get()` 関数の制限により、クロスコレクション参照のテストが困難。本番環境では正しく動作する実装済み
