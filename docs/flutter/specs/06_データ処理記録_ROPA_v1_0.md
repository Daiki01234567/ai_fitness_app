...
| データ項目 | データ型 | 必須/任意 | 目的 | 保存場所 |
|----------|---------|----------|------|---------|
| **氏名(ニックネーム)** | String | 必須 | UI表示、利用者の識別 | Firestore(/users/{uid}) |
| **生年月日** | Timestamp | 必須 | 年齢確認、統計分析 | Firestore(/users/{uid}) |
...
| **フィットネスレベル** | Enum(beginner, intermediate, advanced) | 任意 | 推奨エクササイズ強度の調整 | Firestore(/users/{uid}) |
| **目標** | String[] | 任意 | モチベーション管理、パーソナライズ | Firestore(/users/{uid}) |
| **プロフィール画像** | String(URL) | 任意 | UI表示 | Firebase Authentication(Google/Apple認証時のみ) |

**データ最小化の実装**:
- 必須項目は氏名(ニックネーム)と生年月日のみに限定
- 性別、身長、体重、フィットネスレベル、目標は任意項目として設計
- データ最小化の原則(GDPR第5条1項(c))を実現
- ユーザーは必要最小限の情報のみ提供可能
