...
- [x] 日本語で音声が再生される
- [x] フォームの参考情報が音声で伝えられる（「参考: 良いフォームです」など）
- [x] レップカウントが読み上げられる（「1回」「2回」など）
- [x] 音量調整が設定画面（expo/027）から変更できる
- [x] 読み上げ速度調整が設定画面から変更できる
- [x] ON/OFF切替が設定画面から変更できる
- [x] 音声再生中は重複して再生されない
- [x] すべてのフィードバックに「参考:」プレフィックスが付く（薬機法対応）
...
- `expo_app/services/training/voiceFeedbackService.ts` - 音声再生サービス
- `expo_app/constants/voiceFeedbackMessages.ts` - フィードバックメッセージ定義
- `expo_app/store/settingsStore.ts` - 設定ストア（音量、速度、ON/OFF）
...
|------|----------|
| 2025-12-10 | 初版作成 |
| 2025-12-11 | コード実装完了（VoiceFeedbackService、メッセージ定義） |
