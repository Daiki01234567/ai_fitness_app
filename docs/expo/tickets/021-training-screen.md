...
- [x] カメラプレビューが正しく表示される
- [x] 骨格がリアルタイムでカメラ映像上にオーバーレイ表示される
- [x] リアルタイムフォーム評価が画面上部に表示される（「参考: 良いフォームです」など）
- [x] レップカウント（回数）が正確に表示される
- [x] 経過時間が表示される
- [x] 進捗バー（YouTube風）が表示される
- [x] 一時停止・再開ボタンが機能する
- [x] 終了ボタンで結果画面（expo/022）に遷移する
- [x] 体認識が中断した場合、自動的に一時停止する
- [x] 30秒以上認識できない場合、確認ダイアログを表示する
- [x] 薬機法に準拠した表現（「参考:」プレフィックス）を使用する
...
- `expo_app/app/training/session.tsx` - トレーニング実行画面
- `expo_app/components/training/FeedbackBar.tsx` - フィードバック表示コンポーネント
- `expo_app/components/training/ProgressInfo.tsx` - 進捗情報コンポーネント
- `expo_app/components/training/ControlButtons.tsx` - 操作ボタンコンポーネント
- `expo_app/store/trainingStore.ts` - Zustandトレーニング状態ストア（更新）
...
|------|----------|
| 2025-12-10 | 初版作成 |
| 2025-12-11 | コード実装完了（TrainingSession画面、FeedbackBar、ProgressInfo、ControlButtons） |
