...
- [x] 肘の角度（160-180度）をチェックする`checkElbowAngle`関数を実装
- [x] 手首の高さ（頭より上）をチェックする`checkWristHeight`関数を実装
- [x] ショルダープレスの状態マシン（Down → Pressing → Up → Lowering）を実装
...
- `expo_app/services/evaluation/exercises/ShoulderPressEvaluator.ts` - ショルダープレス評価ロジック
...
|------|----------|
| 2025-12-10 | 初版作成 |
| 2025-12-11 | コード実装完了（ShoulderPressEvaluator）、実機テスト待ち |
