...
- [ ] 膝の角度（90-110度）をチェックする`checkKneeAngle`関数を実装
- [ ] 膝がつま先を越えていないかをチェックする`checkKneeOverToe`関数を実装
- [ ] 背中が真っ直ぐかをチェックする`checkBackStraight`関数を実装
- [ ] スクワットの状態マシン（Standing → Descending → Bottom → Ascending）を実装
