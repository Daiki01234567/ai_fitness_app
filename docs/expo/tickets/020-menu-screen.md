...
- [x] 5種目のカードを縦スクロールで表示
- [x] 各カードに種目名、カテゴリ、難易度、器具の有無、説明を表示
- [x] カードをタップすると、トレーニング実行画面（021）に遷移
- [x] 種目ごとにアイコンまたは画像を表示
- [x] ログイン状態でのみアクセス可能（未ログインは認証画面にリダイレクト）
- [x] ローディング状態を表示
- [x] エラーハンドリング（種目データ取得失敗時）
...
- `expo_app/app/(app)/(tabs)/training.tsx` - メニュー選択画面
- `expo_app/components/training/ExerciseCard.tsx` - 種目カードコンポーネント
- `expo_app/types/exercise.ts` - 種目型定義
- `expo_app/constants/exercises.ts` - 5種目データ定義
...
|------|----------|
| 2025-12-10 | 初版作成 |
| 2025-12-11 | コード実装完了（ExerciseCard, 5種目データ定義、メニュー画面） |
