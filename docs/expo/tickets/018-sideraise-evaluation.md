...
- [x] 腕の挙上角度（肩-肘が水平）をチェックする`checkArmElevation`関数を実装
- [x] 左右対称性（左右の肘の高さがほぼ同じ）をチェックする`checkSymmetry`関数を実装
- [x] サイドレイズの状態マシン（Down → Raising → Up → Lowering）を実装
...
- `expo_app/services/evaluation/exercises/SideRaiseEvaluator.ts` - サイドレイズ評価ロジック
...
|------|----------|
| 2025-12-10 | 初版作成 |
| 2025-12-11 | コード実装完了（SideRaiseEvaluator）、実機テスト待ち |
