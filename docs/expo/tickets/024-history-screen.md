...
- [x] トレーニング履歴が日付降順で表示される
- [x] 各セッションのサマリー（種目、回数、スコア、日時）が表示される
- [x] 種目別フィルター機能が動作する（全て、スクワット、プッシュアップ等）
- [x] 日付範囲フィルター機能が動作する（今週、今月、全期間）
- [x] セッションをタップすると詳細画面に遷移する
- [x] 無限スクロール（ページネーション）が実装されている
...
- `docs/common/specs/11_画面遷移図_ワイヤーフレーム_v1_0.md` - 履歴画面UI設計

## 実装済みファイル一覧

- `expo_app/app/(app)/(tabs)/history.tsx` - 履歴画面
- `expo_app/components/history/SessionCard.tsx` - セッションカードコンポーネント
- `expo_app/components/history/FilterBar.tsx` - フィルターバーコンポーネント
- `expo_app/components/history/index.ts` - コンポーネントエクスポート
- `expo_app/services/training/historyService.ts` - 履歴取得サービス
...
|------|----------|
| 2025-12-10 | 初版作成 |
| 2025-12-11 | コード実装完了（HistoryScreen、SessionCard、FilterBar、historyService） |
