...
Expo版のチケットは、Common（共通バックエンド）のチケットに依存しています。

| Expoチケット | 依存するCommonチケット | 説明 |
|-------------|---------------------|------|
| expo/001 Firebase接続設定 | common/001 Cloud Functions基盤 | Functions基盤完了後にFirebase接続設定が可能 |
| expo/006 認証画面 | common/002 Firestore セキュリティルール | Security Rules完了後に認証画面のテストが可能 |
| expo/006 認証画面 | common/003 Firebase Auth統合 | Auth統合完了後に認証機能が動作 |
| expo/024 履歴画面実装 | common/011 トレーニングセッション保存API | セッションAPI完了後に履歴画面が実装可能 |
| expo/032 Stripe Payment Sheet | common/031 Stripe統合基盤 | Stripe Backend完了後にフロント実装が可能 |
| expo/041 管理者ダッシュボード | common/041 管理者認証API | 管理者認証API完了後にダッシュボード実装が可能 |
